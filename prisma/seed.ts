...
async function main() {
    ...
    const products = await Promise.all(
        Array.from({ length: 10 }).map(() => {
            ...
            const minPrice = faker.number.float({ min: 100_000, max: 300_000, fractionDigits: 0 });
            const maxPrice = minPrice + faker.number.float({ min: 10_000, max: 50_000, fractionDigits: 0 });
        ...
        for (let i = 0; i < 2; i++) {
            const variant = await prisma.productVariant.create({
                data: {
                    ...
                    price: faker.number.float({ min: 100_000, max: 300_000, fractionDigits: 0 }),
